...
export default function TableAdmiReport({
  allUser = [],
  totalPages,
  page,
  pageSize,
  ...
  const handlePagination = async (e) => {
    setPage(1);
    navigate(
      `/dashboard/administrador/historial-revision?pageSize=${
        e.pageSize
      }&page=${1}`
    );
  };
  const handleFilterDate = async () => {
    const urlPag = "/dashboard/administrador/historial-revision";
  ...
  const handleResetFilter = () => {
    ...
    setRefresh(!refresh);
    navigate(
      `/dashboard/administrador/historial-revision?pageSize=${2}&page=${1}`
    );
  };
      ...
      <div className="flex flex-col gap-5 mb-4 max-w-[700px] ">
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <Button
            ...
            onClick={() =>
              handleExport(rows, "Historial atencion pdf")
            }
          >
            EXPORTAR EXCEL
          </Button>
